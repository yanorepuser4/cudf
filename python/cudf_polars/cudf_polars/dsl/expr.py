        ...
        *,
        ...
        if self.name == pl_expr.BooleanFunction.IsNull:
            ...
            return self._distinct(
                ...
                source_value=plc.interop.from_arrow(pa.scalar(True, type=pa.bool_())),  # noqa: FBT003
                target_value=plc.interop.from_arrow(pa.scalar(False, type=pa.bool_())),  # noqa: FBT003
            )
            ...
            return self._distinct(
                column,
                keep=plc.stream_compaction.DuplicateKeepOption.KEEP_NONE,
                source_value=plc.interop.from_arrow(pa.scalar(False, type=pa.bool_())),  # noqa: FBT003
                target_value=plc.interop.from_arrow(pa.scalar(True, type=pa.bool_())),  # noqa: FBT003
            )
        ...
        if indices.obj.null_count():
            ...
            obj = plc.replace.replace_nulls(
                ...
                plc.interop.from_arrow(
                    pa.scalar(n, type=dtypes.to_arrow(indices.obj.data_type()))
                ),
            )
        else:
            ...
            plc.Column.from_scalar(
                plc.interop.from_arrow(
                    pa.scalar(
                        ...
                        type=dtypes.to_arrow(self.dtype),
                    ),
                ),
                1,
            ),
    ...
    def _min(self, column: Column, *, propagate_nans: bool) -> Column:
        if propagate_nans and column.nan_count > 0:
            return Column(
                plc.Column.from_scalar(
                    plc.interop.from_arrow(
                        pa.scalar(float("nan"), type=dtypes.to_arrow(self.dtype))
                    ),
                    1,
                ),
                column.name,
